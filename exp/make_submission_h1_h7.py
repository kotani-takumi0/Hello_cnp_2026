"""exp003: baseline + H1(利益率3本) + H7(CF比率3本) で提出ファイルを作る。
exp001と同一手順（SEED=42, 5fold, OOF閾値, fold平均）にH1+H7特徴を追加。
"""
import os
import numpy as np
import pandas as pd
...
def add_h7(df, X):
    """H7: 現金ベースの投資余力。CF比率3本。"""
    X = X.copy()
    X["営業CFマージン"] = _safe_div(df["営業CF"], df["売上"])
    X["FCF比率"] = _safe_div(df["営業CF"] + df["投資CF"], df["総資産"])
    X["現金ROA"] = _safe_div(df["営業CF"], df["総資産"])
    return X
...
def main():
    ...
    y = tp[TARGET].values
    X = add_h7(train, add_h1(train, tp.drop(columns=[TARGET])))
    ...
    Xte = add_h7(test, add_h1(test, tp_test))[X.columns]
    ...
    out = "submission/submission_h1_h7.csv"
